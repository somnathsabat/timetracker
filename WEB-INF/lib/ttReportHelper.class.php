...
class ttReportHelper {
  ...
  static function getWhere($bean) {
    ...
    if (is_numeric(($bean->getAttribute('category'))))
      $dropdown_parts .= ' and l.category_id = '.$bean->getAttribute('category');
  ...
  static function getItems($bean) {
    ...
    //add category
    //if(is_numeric($bean->getAttribute('category')))
      array_push($fields, "l.category_id as category");
      array_push($fields, "ttc.category as categoryname");
    ...
    //if(is_numeric($bean->getAttribute('category')))
      $left_joins .= "left join tt_category ttc on (ttc.id = l.category_id)";
    if ($include_cf_1) {
      if ($cf_1_type == CustomFields::TYPE_TEXT)
...
// $mdb2->setOption('debug', true);
    ...
    $res =  $mdb2->query($sql);
    //print_r($res->getColumnNames());exit;
// return $res;
    //echo $res->numCols();exit;
...
//print_r($val);
      ...
      if ('no_grouping' != $group_by_option && 'categoryname' != $group_by_option ) {
        //echo "1";die;
      ...
      $report_items[] = $row;
       ...
       /*echo '<pre>';
      print_r($report_items);exit;*/
      //return 1;
     $var =  mdb2::errorMessage($bean);
     /*if($var)
     {
       print_r($var);exit;
     }*/
