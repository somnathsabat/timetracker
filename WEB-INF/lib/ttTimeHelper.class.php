...
class ttTimeHelper {
  ...
  static function insert($fields)
  {
    ...
    if ($duration) {
      //echo "hi";exit;
      $sql = "insert into tt_log (timestamp, user_id,start, date, duration, client_id, project_id,category_id, task_id, invoice_id, comment, billable $status_f) ".
        "values ('$timestamp', $user_id,'$start', ".$mdb2->quote($date).", '$duration', ".$mdb2->quote($client).", ".$mdb2->quote($project).",".$mdb2->quote($category).", ".$mdb2->quote($task).", ".$mdb2->quote($invoice).", ".$mdb2->quote($note).", $billable $status_v)";
      ...
      $sql = "insert into tt_log (timestamp, user_id, date, start, duration, client_id, project_id,category_id, task_id, invoice_id, comment, billable $status_f) ".
        "values ('$timestamp', $user_id, ".$mdb2->quote($date).", '$start', '$duration', ".$mdb2->quote($client).", ".$mdb2->quote($project).", ".$mdb2->quote($category).", ".$mdb2->quote($task).", ".$mdb2->quote($invoice).", ".$mdb2->quote($note).", $billable $status_v)";
      $affected = $mdb2->exec($sql);
      if (is_a($affected, 'PEAR_Error'))
        return false;
    }
  ...
  static function update($fields)
  {
    ...
    if ($duration) {
      $sql = "UPDATE tt_log set start = NULL, duration = '$duration', client_id = ".$mdb2->quote($client).", project_id = ".$mdb2->quote($project).",category_id = ".$mdb2->quote($category).", task_id = ".$mdb2->quote($task).", ".
      ...
      $sql = "UPDATE tt_log SET start = '$start', duration = '$duration', client_id = ".$mdb2->quote($client).", project_id = ".$mdb2->quote($project).",category_id = ".$mdb2->quote($category).", task_id = ".$mdb2->quote($task).", ".
        "comment = ".$mdb2->quote($note).", billable = $billable, date = '$date' WHERE id = $id";
      $affected = $mdb2->exec($sql);
      if (is_a($affected, 'PEAR_Error'))
        return false;
